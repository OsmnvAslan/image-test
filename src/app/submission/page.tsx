  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16 text-neutral-800">
      ...
      <Section index="1" title="The product">
        ...
        <figure className="my-6">
          ...
          <figcaption className="mt-2 text-sm text-neutral-500">
            One generated post: real image (Pollinations) + AI social copy overlaid
            as crisp HTML text.
          </figcaption>
        </figure>
        ...
        <ul className="ml-5 list-disc space-y-2">
          ...
          <li>
            <b>Style consistency</b> — a single vision-derived <i>style fingerprint</i>{" "}
            is computed <b>once</b> per batch and reused verbatim for every product;
            a per-product description supplies the subject. Same style spine,
            different product per card.
          </li>
          <li>
        ...
        <p className="mt-4 rounded-md bg-neutral-100 p-3 text-sm text-neutral-600">
          Run target is a single local process (<code>next start</code>), not
          serverless: the in-memory store + long-lived SSE need one persistent
          process. <code>npm install &amp;&amp; npm run build &amp;&amp; npm start</code> →{" "}
      ...
      <Section index="2" title="How it was built">
        <h3 className="mb-2 mt-2 font-semibold text-neutral-900">
          How I used AI
        </h3>
        <p>
          I worked in <b>agent-teams</b> mode: acting as team lead, I wrote a{" "}
          <code>PLAN.md</code> with exact contracts (shared types, 3 HTTP endpoints,
          the generation- and style-module interfaces), then deployed{" "}
        ...
        <ul className="ml-5 mt-2 list-disc space-y-2">
          <li>
            ...
            vision model, every product got the same generic description → identical
            prompts → identical images. The AI had designed a fallback that was
            “correct but useless.”
          </li>
          <li>
            <b>Model drift:</b> the model in the plan
            (<code>gemini-2.0-flash-exp:free</code>) had vanished from OpenRouter’s
            free list; later the chosen <code>gemma</code> model began returning{" "}
            <code>429</code>. I overrode the AI’s pick and switched to{" "}
          ...
          <li>
            <b>Review fixes:</b> a self-review found that the fallback copy collapsed
      ...
      <Section index="3" title="The code">
        ...
        <p className="mt-4 text-sm text-neutral-600">
          ...
          SSE), <code>src/app/page.tsx</code> (UI). Docs:{" "}
          <code>README.md</code> (run), <code>DECISIONS.md</code> (built / cut /
          why), <code>PLAN.md</code> (contracts).
        </p>
      </Section>
...
const TOOLS: { name: string; use: string }[] = [
  ...
  { name: "Claude Opus 4.8", use: "Team-lead reasoning, prompts, review, debugging" },
  {
  ...
  { name: "Together AI FLUX (free)", use: "Optional second image provider (failover)" },
  { name: "Next.js 14 + TS + Tailwind", use: "App stack (App Router, API routes, SSE)" },
